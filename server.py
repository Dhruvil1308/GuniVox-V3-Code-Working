...
async def trigger_call(req: CallRequest):
    try:
        # Determine public URL dynamically from local Ngrok API
        ...
        if not ngrok_url:
            try:
                ...
                logger.warning(f"Could not auto-detect Ngrok URL: {e}")

        if not ngrok_url:
            raise HTTPException(status_code=500, detail="Could not find Ngrok URL. Make sure Ngrok is running!")
...
if __name__ == "__main__":
    import uvicorn
    logger.info(f"🚀 GuniVox Backend Running...")
